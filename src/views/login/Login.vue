    ...
    <div class="login-box" v-if="!selectEnterPrise">
      <div class="toggle-box">
        <a-tooltip placement="left" v-show="loginType === 'captcha'" title="密码登录">
          <img src="./assets/computer.png" v-show="loginType === 'captcha'" alt="密码登录" class="toggle-icon" @click="toggleLoginType">
        </a-tooltip>
        <a-tooltip placement="left" v-show="loginType === 'password'" title="验证码登录">
          <img src="./assets/phone.png" v-show="loginType === 'password'" alt="验证码登录" class="toggle-icon" @click="toggleLoginType">
        </a-tooltip>
        <div class="mask"></div>
      </div>
      ...
      <!-- 密码登录 -->
      <a-form-model ref="passwordForm" :model="passwordForm" :rules="passwordFormRules" v-show="loginType === 'password'">
        ...
        <a-form-model-item prop="captcha">
          <a-input placeholder="请输入图片验证码" v-model="passwordForm.captcha" allowClear autocomplete="off" @pressEnter="login">
        ...
        <a-form-model-item prop="captcha">
          <a-input placeholder="请输入验证码" v-model="captchaForm.captcha" allowClear autocomplete="off" @pressEnter="login">
            <a-button
              ...
              :disabled="loading"
              @click="getSMSCaptcha"
              v-text="this.btnText || '获取验证码'"></a-button>
          </a-input>
        </a-form-model-item>
      </a-form-model>
      <div class="forgetPwd">
      ...
      <p class="footer-link">登录代表您已同意<a href="#" @click="openAgreement(1)">《用户协议》</a> 和 <a href="#" @click="openAgreement(2)">《隐私政策》</a></p>
    </div>
...
export default {
  ...
  methods: {
    ...
    loginSuccess (res) {
      this.GetRelatedCompanyList().then(dataList => {
        const length = dataList.length
        if (length === 1) {
          this.selectEnterPrise = false
          const id = dataList[0]?.id
          this.SetCompanyId(id).then(res => {
            this.GetCanUseSystemList(id).then(res2 => {
              const appIds = (this.$map.appList || []).map(item => item.id)
              const canUseAppList = res2.dataList.filter(item => appIds.includes(item.id))
              if (canUseAppList.length === 1) {
                const appId = canUseAppList[0].id
                if (process.env.NODE_ENV === 'development') {
                  window.location.replace(`${(this.$map.devSystemUrlMap || {})[appId]}/?token=${this.token}&companyId=${id}`)
                } else {
                  window.location.replace(`${canUseAppList[0].frontpath}/?token=${this.token}&companyId=${id}`)
                }
              } else {
                this.$router.push('/')
              }
